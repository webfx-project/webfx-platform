...
public class PushbackInputStream extends FilterInputStream {
    ...
    public int read(byte[] buffer, int offset, int length) throws IOException {
        ...
        if (offset > buffer.length || offset < 0) {
            ...
            throw new ArrayIndexOutOfBoundsException("Offset " + offset + " out of bounds expected 0 to " + buffer.length); //$NON-NLS-1$
        }
        if (length < 0 || length > buffer.length - offset) {
            // luni.18=Length out of bounds \: {0}
            //throw new ArrayIndexOutOfBoundsException(Messages.getString("luni.18", length)); //$NON-NLS-1$
            throw new ArrayIndexOutOfBoundsException("Length " + length + " out of bounds expected 0 to " + buffer.length); //$NON-NLS-1$
        }
    ...
    public void unread(byte[] buffer, int offset, int length)
        ...
        if (offset > buffer.length || offset < 0) {
            // luni.12=Offset out of bounds \: {0}
            //throw new ArrayIndexOutOfBoundsException(Messages.getString("luni.12", offset)); //$NON-NLS-1$
            throw new ArrayIndexOutOfBoundsException("Offset " + offset + " out of bounds, expected 0 to " + buffer.length);
        }
        if (length < 0 || length > buffer.length - offset) {
            // luni.18=Length out of bounds \: {0}
            //throw new ArrayIndexOutOfBoundsException(Messages.getString("luni.18", length)); //$NON-NLS-1$
            throw new ArrayIndexOutOfBoundsException("Length " + offset + " out of bounds, expected 0 to " + buffer.length);
        }
        if (buf == null) {
            // luni.24=Stream is closed
            //throw new IOException(Messages.getString("luni.24")); //$NON-NLS-1$
