...
public interface BrowsingHistory {
     ...
     */
    default void push(String path, ReadOnlyAstObject state) { push(createPathStateLocation(path, state)); }
     ...
     * @param state A state object associated with this history entry
     */
    default void replace(String path, ReadOnlyAstObject state) { replace(createPathStateLocation(path, state)); }
